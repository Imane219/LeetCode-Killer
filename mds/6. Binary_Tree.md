...
It is quite like given an array to find subarray nums of targeted sum. We enumerate the left boundary and for each left enumerate the right boundary. $O(n^2)$
...
==Prefix method==: we can construct a dict to store the prefix and the time that it appears. `dic[prefix]: freq`. Thus we only need to enumerate the ==right boundary== `r`. For each [0:r+1], we need to find the possible `l` that [l:r+1] sums up to target -> find the possible `l` that [0:l] sums up to `prefix_r - target`.
